...
<%
		...
		try {
			...
			if(conn != null){
				sql = "select mem_name, mem_hp, mem_email, mem_hobby, mem_ssn1, mem_ssn2, mem_zipcode, mem_address1, mem_address2, mem_address3 from tb_member where mem_index=? and mem_userid=?";
...
<body>
	...
	<form action="./info_ok.jsp" method="post" name="regform"
		id="regform" onsubmit="return sendit()">
		<p>
			아이디 : <%=userid %>
		</p>
		<p>
		...
		<p>
			휴대폰 번호 : <input type="text" name="hp" id="hp" value="<%=hp %>" />
		</p>
		<p>
		...
		<p>
			우편번호 : <input type="text" id="sample6_postcode" name="zipcode" value="<%=zipcode %>" /> <input
				type="button" onclick="sample6_execDaumPostcode()" value="우편번호 검색" />
		</p>
		<p>
			주소 : <input type="text" id="sample6_address" name="address1" value="<%=address1 %>" /> 상세
			주소 : <input type="text" id="sample6_detailAddress" name="address2" value="<%=address2 %>" />
		</p>
		<p>
			참고항목 : <input type="text" id="sample6_extraAddress" name="address3" value="<%=address3 %>" />
		</p>
		<p>
			<input type="submit" value="수정완료" />
			<input type="reset" value="다시작성" />
			<input type="button" value="뒤로" onclick="history_back()">
		</p>
	</form>
</body>
</html>
